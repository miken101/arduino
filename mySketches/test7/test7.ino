...
void loop() {
  static int pattern = 0, lastReading1,lastReading2,lastReading3;
  int reading1 = digitalRead(button1);
  int reading2 = digitalRead(button2);
  int reading3 = digitalRead(button3);
  if(lastReading1 == HIGH && reading1 == LOW){
  ...
  else if(lastReading2 == HIGH && reading2 == LOW){
    pattern = 1 ; // change pattern number
    patternInterval = intervals[pattern]; // set speed for this pattern
    wipeClear(); // clear out the buffer 
    delay(50); // debounce delay
  }
    else if(lastReading3 == HIGH && reading3 == LOW){
